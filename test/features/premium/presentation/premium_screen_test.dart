...
final appUser = AppUser.fromJson({
  'id': 'u1', 'displayName': 'User', 'isPremium': false, 'premiumTrialUsed': false,
  'preferences': {'completedOnboarding': true}, 'counts': {},
});
PayosPayment makePayment(PaymentStatus status) => PayosPayment(
  id: 'p1', planId: PremiumPlanId.quarter, orderCode: 123, amount: 99000,
  currency: 'VND', status: status, checkoutUrl: 'https://pay.payos.vn/web/p1',
);
class _Repository implements PremiumRepositoryContract {
  @override Future<List<PremiumPlan>> plans() async => const [
    PremiumPlan(id: PremiumPlanId.month, name: 'Gói tháng', displayPrice: '39k/tháng', amount: 39000, durationMonths: 1),
    PremiumPlan(id: PremiumPlanId.quarter, name: 'Gói 3 tháng', displayPrice: '99k/3 tháng', amount: 99000, durationMonths: 3),
  ];
  @override Future<PayosPayment> createPayment(PremiumPlanId planId) async => makePayment(PaymentStatus.pending);
  @override Future<PayosPayment> payment(int orderCode) async => makePayment(PaymentStatus.paid);
  @override Future<AppUser> claimTrial() async => appUser;
}
class _Launcher implements CheckoutLauncher {
  Uri? uri;
  @override Future<bool> open(Uri uri) async { this.uri = uri; return true; }
}
void main() {
  testWidgets('selects plan, launches PayOS and renders payment status', (tester) async {
    ...
    await tester.pumpWidget(ProviderScope(child: MaterialApp(home: PremiumScreen(controller: controller, user: appUser))));
